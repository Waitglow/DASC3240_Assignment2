...
**Why this animation type (and why it's better than a static plot):** I chose a KDE heatmap over a simple scatter plot animation because point-based movements can be noisy and fail to identify population "cores". A heatmap allows us to visualize the "Aurora-like" shifts in species concentration, providing a clearer biological narrative of how the dominant fish populations reorganize themselves spatially.

Crucially, a static map of this decade-long survey would suffer from severe **overplotting**, making it impossible to distinguish yearly trends. This animation provides a **clear temporal narrative**, transforming static data into a fluid 'occupancy probability' field. It allows the viewer to witness the movement of biodiversity hotspots over time, which provides an intuitive understanding of ecosystem adaptation that static snapshots simply cannot capture.

**Strengths:**

-   **Spatiotemporal Continuity**: Successfully bridges the gap between spatial distribution and temporal change.

-   **Scientific Consistency**: Implements a global normalization strategy (`global_max`), ensuring color intensity remains scientifically comparable across years.

-   **Storytelling Impact**: Highlighting the resilience and migration patterns of species like *Pleuragramma antarcticum*.

**Weaknesses:**

-   **Computational Intensity**: Rendering high-resolution KDE for multiple years is memory-intensive.

-   **Sampling Gaps**: The density estimation can be sensitive to years with significantly lower sampling effort.
:::
